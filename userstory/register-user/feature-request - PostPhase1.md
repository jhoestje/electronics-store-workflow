...
**Acceptance Criteria**:
- [ ] A "Register" button/link is visible in the Navbar when no user is logged in
- [ ] Clicking "Register" navigates to `/register`
- [ ] The form has Username, Email, and Password fields
- [ ] The form has a submit button labeled "Register"
- [ ] The form has a link to the Login page for users who already have an account
...
**Acceptance Criteria**:
- [ ] Submitting valid credentials creates a new user account
- [ ] A real JWT token is returned and stored in `localStorage`
- [ ] The user is automatically authenticated and redirected to the home page
- [ ] The Navbar shows the logged-in state (user name / profile link)
...
- [ ] Empty or invalid email shows "Valid email is required"
- [ ] Empty password shows "Password is required"
- [ ] Weak password shows "Password must be 8+ characters with uppercase, digit, and special character"
...
**Acceptance Criteria**:
- [ ] `JwtService` generates signed JWT tokens with configurable secret and expiration
- [ ] `JwtAuthFilter` validates tokens on incoming requests
- [ ] `AuthService.register()` and `AuthService.login()` return real JWT tokens
...
**Acceptance Criteria**:
- [ ] Duplicate username → `400 Bad Request` with `{ "error": "Username already exists" }`
- [ ] Duplicate email → `400 Bad Request` with `{ "error": "Email already exists" }`
- [ ] Validation failures → `400 Bad Request` with field-level error messages
...
- **Next Phase**: Phase 1 — Planning & Design
- **Password Strength Rules (User)**: 8+ chars, uppercase, digit, special character — enforced on backend (`@Pattern`) and frontend (Yup)
- **Pending User Input**: None — all questions answered
- **Phase 0 Gate**: Awaiting user approval
